...
abstract class REST_Controller extends CI_Controller
{
	...
	*/
	...
	public function __construct()
	{
        ...
        /*
         * Novo método de recuperação de dados para a API
         * Possibilita utilizar parametros sem valor para melhor visibilidade da API
         * Problema: Todos os parametros viram chave|valor
         *
         *
         * Versão 1.0 - Estável
         */
        $explodeURI = explode('/', $this->uri->uri_string());
        $args = [];

        foreach ($explodeURI as $key=>$value){
            $args += (isset($explodeURI[$key + 1])) ? [$value => $explodeURI[$key + 1]] : [];
        }
	...
	/**
     * A sequencia de quatro funções abaixo deve ser refatorada.
     * Edição estável; 16/09/2013
     *
     * Pode dar conflitos com códigos mais elaborados e com mais métodos.
     *
     * @caioreis
	 */
	public function get($key = NULL, $xss_clean = TRUE)
	{
        ...
        foreach ($this->privateMethods as $method)
        {
            if (!isset($this->_get_args[$key]) OR $this->_get_args[$key] == $method)
                return false;
        }
    ...
    {
        foreach ($this->_get_args as $key=>$value)
        {
    ...
    {
        if (!$this->verifyPrivateMethods($controller))
            return false;
    ...
    function verifyPrivateMethods($key)
    {
        foreach ($this->privateMethods as $method)
        {
            if (!isset($this->_get_args[$key]) OR $this->_get_args[$key] == $method OR $this->getSegment(3) == $method)
                return false;
        }
    ...
    public function getSegment($key = null)
    {
        ...
        if ($this->uri->segment(1) == 'api' OR $this->uri->segment(0) == 'api')
        {
            $key = $key + 1;
        }

        return ($this->uri->segment($key)) ? $this->uri->segment($key) : false;
    }
	...
	/**
	 * Check if there is a key associated to the login
	 *
	...
	protected function _check_login($username = '', $password = NULL)
	{
		...
		$keys_table = strtolower($this->config->item('rest_keys_table'));
        $key_column = strtolower($this->config->item('rest_key_column'));
        $secret_column = strtolower($this->config->item('rest_secret_column'));

        $this->ci = & get_instance();

        $query = $this->ci->db->get_where($keys_table, [$key_column => $username, $secret_column => $password]);
